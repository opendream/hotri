...
echo "<h1>Bulk summary</h1>";


if ($_GET['act'] == 'cleartemp') {
  $bl = new BulkLookupQuery();
  $bl->clearDoneQueue('manual_list');
  echo '<h5 id="updateMsg">Hidden items (no copy) has been removed from failed list.</h5>';
}
...
if (!empty($_GET['del'])) { // Action: del
  ...
  echo '<h5 id="updateMsg">ISBN ' . $isbn . ' has been removed from failed list.</h5>';
}
...
foreach ($rows as $row) {
  ...
  echo "<tr><td>{$row['isbn']}</td><td>{$row['hits']}</td><td>{$row['created']}</td><td><a href=\"../catalog/biblio_new.php?isbn={$row['isbn']}&hits={$row['hits']}\">add</a></td>" .
   "<td><a href=\"?del={$row['isbn']}\" onclick=\"return confirm('Are you sure to remove ISBN: {$row['isbn']}?')\">remove</a></td>" .
